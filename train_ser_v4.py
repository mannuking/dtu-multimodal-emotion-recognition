"""
train_ser_v4.py - Phase 1 architecture for the 82% target.

Three changes vs train_ser_enhanced.py (which is the v2/v3 baseline at ~68%):

  1. wav2vec2-large (317M params) instead of wav2vec2-base (90M params).
     Pre-trained on 16kHz LibriSpeech + CommonVoice + Switchboard + Fisher
     — captures much richer prosodic features relevant to emotion. Expected
     +5-8pp on SER benchmarks vs wav2vec2-base.

  2. Supervised Contrastive (SupCon) auxiliary loss on top of CE.
     Reference: Khosla et al. "Supervised Contrastive Learning" (NeurIPS 2020).
     Adds an InfoNCE-style loss on the 128-dim projected embeddings, pulling
     same-class samples together and pushing different-class apart. Works
     particularly well when class boundaries are blurry (fear vs sad,
     neutral vs calm). Expected +2-4pp.

  3. Stronger head: bidirectional attention pooling instead of single-head
     attention, with layer norm + GELU MLP. Adds capacity to the head
     without overfitting (the encoder is the bottleneck).

  4. Gradient checkpointing on the encoder so wav2vec2-large fits in
     40GB A100 memory. Trade-off: ~30% slower per step, but enables
     batch size 8-12 (we used 24 for base).

  5. Mixup disabled by default (conflicts with SupCon — mixup interpolates
     labels which makes contrastive pair mining ambiguous). SpecAugment
     still on.

Ensemble: train 3 seeds (42/43/44) in parallel via sbatch, then ensemble.

Usage:
  uv run python train_ser_v4.py --seed 42
  uv run python ensemble_v4.py --seeds 42 43 44 --n-tta 5

Expected: 76-82% test acc on the combined SER dataset (vs 68% v2 baseline).
"""
...
# wav2vec2-large config (vs -base which was 768-dim, 12 layers)
# -large is 1024-dim hidden, 24 transformer layers, ~317M params.
...
LR_HEAD = 1e-4
WEIGHT_DECAY = 1e-5
SUPCON_WEIGHT = 0.5      # 0.5 * SupCon + 1.0 * CE (combined)
SUPCON_TEMP = 0.07       # standard SupCon temperature

# HF cache path (same as train_ser_enhanced.py — model lives in ~/.cache/huggingface/...)
LARGE_MODEL_PATH = os.path.expanduser(
    f"~/.cache/huggingface/hub/models--facebook--wav2vec2-large/"
    f"snapshots/6235644f62f8a7e7b6e8d4e7e8e8e8e8e8e8e8e8"
)
...
class Wav2Vec2LargeExtractor(nn.Module):
    """wav2vec2-large with last N layers unfrozen + gradient checkpointing.

    Key differences from Wav2Vec2FeatureExtractor (the v2/v3 base version):
    - Uses -large (1024-dim, 24 layers, 317M params) instead of -base
    - Gradient checkpointing enabled by default — required to fit in 40GB
    - By default freezes everything except last 6 layers (24-layer model
      is much more prone to overfitting on 9.8k samples than 12-layer base)
    """

    def __init__(self, model_path: str, num_unfrozen_layers: int = 6,
    ...
    def get_layer_param_groups(self, base_lr, weight_decay, decay_rate=0.95):
        """Layer-wise LR decay (same ULMFiT-style schedule as v3)."""
        param_groups = []
        for i, layer in enumerate(self.encoder.encoder.layers):
...
class StrongSERHead(nn.Module):
    """Bidirectional attention pooling + MLP head for v4.

    Replaces v3's single-head attention pool with:
    - 2-head attention pool (averaged), capturing both forward + backward
      dependencies in the time axis
    - LayerNorm + GELU MLP (256 hidden, 128 projection)
    - Dual output: classification logits (7-dim) + projection embeddings
      (128-dim, used for SupCon loss)
    """

    def __init__(self, in_channels: int = 1024, num_classes: int = 7,
        ...
        # Conv backbone (4 residual blocks, smaller than v3 since encoder is bigger)
        ...
        self.cls_head = nn.Sequential(
            nn.LayerNorm(256),  # 128 forward + 128 backward
            nn.Dropout(dropout),
            nn.Linear(256, 256),
            nn.GELU(),
            nn.Dropout(dropout),
    ...
    def forward(self, x):
        ...
        attn_fwd = F.softmax(self.attn_pool(x), dim=1)
        pooled_fwd = (x * attn_fwd).sum(dim=1)  # (B, 128)

        # Backward: reverse along time then pool
        x_rev = torch.flip(x, dims=[1])
        attn_bwd = F.softmax(self.attn_pool_bwd(x_rev), dim=1)
        pooled_bwd = (x_rev * attn_bwd).sum(dim=1)  # (B, 128)

        pooled = torch.cat([pooled_fwd, pooled_bwd], dim=-1)  # (B, 256)
...
class SupConLoss(nn.Module):
    ...
    From Khosla et al. NeurIPS 2020. For each anchor, all same-class samples
    in the batch are positives, all others are negatives. Uses cosine
    similarity with temperature scaling.

    Requires features L2-normalized before similarity computation.
    """
    ...
    def forward(self, features: torch.Tensor, labels: torch.Tensor) -> torch.Tensor:
        # features: (B, D) — should be L2-normalized before this
        # labels: (B,)
        ...
        # Mean log-prob over positives (handle case of no positives)
...
class ClassBalancedCE(nn.Module):
    """Cross-entropy with per-class weights (inverse-frequency)."""
...
class SpecAugment(nn.Module):
    def __init__(self, time_mask_max=128, freq_mask_max=256,
...
def train_v4(seed: int = SEED_DEFAULT,
             num_unfrozen_layers: int = 6,
             n_epochs: int = NUM_EPOCHS,
             supcon_weight: float = SUPCON_WEIGHT,
             batch_size: int = BATCH_SIZE):
    """Train wav2vec2-large + SupCon head."""
    ...
    print(f"\n========== v4 Training run: seed={seed}, unfrozen={num_unfrozen_layers}/24 ==========")
    print(f"   device: {device}, GPUs: {torch.cuda.device_count()}")
    print(f"   supcon_weight={supcon_weight}, batch_size={batch_size}, n_epochs={n_epochs}")

    # ---- Load wav2vec2-large ----
    # Use local cache if available, else download
    local_path = os.path.expanduser(
        "~/.cache/huggingface/hub/models--facebook--wav2vec2-large"
    )
    # Try common snapshot subdirs; HF cache structure varies by version
    ...
    # Subject-disjoint split (same as v3: split_seed=42 fixed across ensemble)
    split_seed = 42
    ...
    # ---- Save best ----
    if best_state:
        ckpt_path = os.path.join(CHECKPOINT_DIR, f"ser_v4_best_seed{seed}.pt")
    ...
    with torch.no_grad():
        for x, y in test_loader:
            x = x.to(device, non_blocking=True)
            probs = predict_tta(x, n_passes=5)
    ...
    summary = {
        "model": "wav2vec2-large (last 6 layers unfrozen, grad ckpt) + bidirectional attn pool + SupCon + ClassBalancedCE + SpecAugment + 50 epochs cosine + 5-pass TTA",
        "seed": int(seed),
    ...
    summary_path = os.path.join(CHECKPOINT_DIR, f"ser_v4_training_summary_seed{seed}.json")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Train SER v4 (wav2vec2-large + SupCon)")
    parser.add_argument("--seed", type=int, default=SEED_DEFAULT)
    parser.add_argument("--num-unfrozen-layers", type=int, default=6,
                        help="Last N layers to unfreeze (default: 6 of 24)")
