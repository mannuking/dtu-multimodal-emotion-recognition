"""
ensemble_evaluate_v4.py - Average v4 (wav2vec2-large + SupCon) checkpoints.

Loads N checkpoints (one per seed), runs TTA on each, averages softmax
probabilities, reports the ensemble accuracy / F1 / classification report.

Same pattern as ensemble_evaluate.py but for the v4 architecture
(StrongSERHead returns (logits, proj) instead of just logits).

Usage (from HPC, must go through sbatch):
...
def predict_tta(model, feature_extractor, x, n_passes=5):
    ...
    for i in range(n_passes):
        with torch.no_grad():
            ...
            logits, _ = model(feats)  # discard proj embedding
...
def main():
    ...
    # ---- Load wav2vec2-large + StrongSERHead ----
        ...
        for snap in os.listdir(local_path):
            ...
            if os.path.exists(os.path.join(candidate, "pytorch_model.bin")) or \
               os.path.exists(os.path.join(candidate, "model.safetensors")):
                ...
                break
    feature_extractor = Wav2Vec2LargeExtractor(model_path, num_unfrozen_layers=24).to(device).eval()
    ...
    for seed in args.seeds:
        print(f"\n--- Loading seed {seed} v4 checkpoint ---")
    ...
    print(f"\n============================================================")
    print(f"V4 ENSEMBLE RESULTS ({len(args.seeds)} seeds, {args.n_tta}-pass TTA)")
    print(f"============================================================")
    ...
    print(f"\nV4 ENSEMBLE TEST ACCURACY: {ensemble_acc:.4f}")
    print(f"V4 ENSEMBLE TEST MACRO-F1: {ensemble_f1:.4f}")
    ...
    summary = {
        "method": f"v4 ensemble: wav2vec2-large + SupCon + bi-attn pool, {len(args.seeds)}-seed avg, {args.n_tta}-pass TTA",
        "seeds": args.seeds,
    ...
    summary_path = os.path.join(CHECKPOINT_DIR, "ser_v4_ensemble_summary.json")
    with open(summary_path, "w") as f:
        json.dump(summary, f, indent=2)
    print(f"\nSaved v4 ensemble summary to {summary_path}")
